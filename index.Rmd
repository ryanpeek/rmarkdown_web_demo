...
There are a few pieces that all need to fit together to create a website using this framework. The main pieces are as follows:
  ...
  2.  `.Rmd`: These are your Rmarkdown files that will be used to render your  webpage content, each `.Rmd` should have a correpsonding `.html`. When we knit, the `.Rmd` will create file named the same, but with a `.html` extension.

**Additional Options**

 - `.nojekyll` (*optional*: this tells github how to build page)
